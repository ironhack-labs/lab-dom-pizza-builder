...
function renderWhiteSauce() {
  ...
  document.querySelectorAll('.sauce.sauce-white').forEach((wSauce) => {
    if (state.whiteSauce) {
...
function renderGlutenFreeCrust() {
  ...
  document.querySelectorAll('.crust').forEach((crust) => {
    ...
    } else if (crust.classList.contains('crust-gluten-free')) {
...
function renderPrice() {
  ...
  document.querySelectorAll('.panel.price>ul>li').forEach((li) => {
    for (key in state) {
      if (key == li.getAttribute('id').split('Cost')[0]) {
        if (state[key] == true) {
  ...
  for (key in state) {
    if (state[key] == true) {
      total = ingredients[key].price + total;
    } else {
      // total = ingredients[key].price - total
    }
  }
