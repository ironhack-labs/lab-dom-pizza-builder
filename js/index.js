...
function renderPrice() {
  ...
  for (const item in ingredients) {
    if (ingredients[item]) {
      const itemObj = ingredients[item];
      if (state[item]) {
        totalPrice = totalPrice + itemObj.price;
        const listElement = document.createElement('li');
        const innerText = document.createTextNode(`$${itemObj.price} ${itemObj.name}`);
        listElement.appendChild(innerText);
        ulElement.appendChild(listElement);
      }
    }
  }
  const strongElement =  document.querySelector(".panel.price strong");
  if (strongElement) {
