...
function renderPrice() {
  ...
  const pricePanelList=document.querySelector('.price ul');
  let total=0;
  // console.log(pricePanelList)
  let toppings='';
  for(let key in state){
    if(state[key]){
      toppings+=`<li>$${ingredients[key][price]} ${ingredients[key]}<li>`;
      total+=ingredients.key.price;
    }
  }

  pricePanelList.innerHTML(toppings)
...
WhiteSauceBtn.addEventListener('click',()=>{
...
glutenFreeCrustBtn.addEventListener('click',()=>{
