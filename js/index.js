...
function renderGlutenFreeCrust() {
  ...
  document.querySelectorAll('.crust').forEach(oneGluten =>{
    if(state.glutenFreeCrust){
      oneGluten.classList.add('crust-gluten-free');
    }
    else{
      oneGluten.classList.remove('crust-gluten-free')
    }
  })
...
function renderPrice() {
  ...
  /*let text = document.getElementsByClassName(panel [0])
  parent.remove(text);
  if(state.pepperoni){
    const newList = document.createElement('l');
    newList.innerText = "+ $1 pepperoni";
    const parent = document.getElementsByClassName("panel price")[0];
  ...
  for (let element in state) {
    if (state[element]) {
      ...
      list += `<li>$${ingredients[element].price}  ${ingredients[element].name}</li>`
    }
    }
    document.querySelector('.price ul').innerHTML = list
    document.querySelector('aside strong').innerHTML = `$${sum}`
  }
  
  //const ul = event.currentTarget.parentNode;  // below 2 lines to remove from the list
