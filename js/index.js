...
//classlist toggle + classlist remove (w classlist add)
...
let gfBtn = document.querySelector('.btn-crust')
...
if (state.glutenFreeCrust) {
  gfBtn.classList.add('active')
} else gfBtn.classList.remove('active')

if (state.whiteSauce) {
  whiteSauceBtn.classList.add('active')
} else whiteSauceBtn.classList.remove('active')
...
function renderPrice() {
  ...
  for (let ingredient in ingredients){
    if(state[ingredient]) {
      ingList.innerHTML += `<li>$${ingredients[ingredient].price} ${ingredients[ingredient].name}</li>`
      newPrice += ingredients[ingredient].price;
    } 
  }

  document.querySelector('strong').innerHTML = `$${newPrice}`;
