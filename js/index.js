...
function renderButtons() {
  ...
  let btn = document.querySelectorAll('.btn')
    ...
    (Object.values(state)[i]) ? btn[i].classList.add('active') : btn[i].classList.remove('active')
  }
}

function renderPrice() {
  // Iteration 4: change the HTML of `<aside class="panel price">`
  let cart = document.querySelectorAll(".panel.price li");
  let total = document.querySelector(".panel.price strong");
  let totalPrice = basePrice;
  for (let i = 0; i < cart.length; i++) {
    if (Object.values(state)[i]) {
      totalPrice += Object.values(ingredients)[i].price;
