...
function renderGlutenFreeCrust() {
  ...
  document.querySelectorAll('.crust').forEach(crst => {
...
function renderButtons() {
  // Iteration 3: add/remove the class "active" of each `<button class="btn">`
     document.querySelectorAll('.btn').forEach(act => {
  ...
  // Iteration 4: change the HTML of `<aside class="panel price">`
   ...
   for (let key in state) {
     if(state[key]) {
       ...
       elem.textContent = ` ${ingredients[key].name} ${ingredients[key].price}$`
       document.querySelector('.price ul').appendChild(elem)
       console.log(document.querySelector('.price ul'))
