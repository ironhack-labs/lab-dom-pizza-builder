...
function renderButtons() {
    ...
    for (let key of Object.keys(ingredients)) {
        if (state[key]) {
            document.querySelector(ingredientClass[key]).classList.add('active');
        } else {
            document.querySelector(ingredientClass[key]).classList.remove('active');
        }
    }
}
...
function renderPrice() {
    ...
    for (let key of Object.keys(ingredients)) {
        if (state[key]) {
            ...
            child.textContent = `$${ingredients[key].price} ${ingredients[key].name}`
            list.appendChild(child);
        }
    }
    document.querySelector('body > aside > b').innerText = `$${basePrice} cheese pizza`;
    document.querySelector('.price > *:last-child').innerText = `$${total}`;
}
