...
function renderButtons(button) {
    ...
    if (button === undefined) {
        for (key in state) {
            let tagButton = nameButton[key];

            var elementButton = document.querySelector(tagButton);
            var elementButtonActive = elementButton.classList.contains("active");
...
function renderPrice() {
    ...
    let liAllelement = document.querySelectorAll(".panel.price ul li")
    ...
    for (key in state) {
        if (state[key]) {
            let element = ingredients[key];
            let liElement = document.createElement("li")

            price += element.price;

            liElement.innerText = `$ ${element.price} - ${element.name}`
