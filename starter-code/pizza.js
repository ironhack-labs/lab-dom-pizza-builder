...
function renderMushrooms() {
  ...
  document.querySelectorAll('.mushroom').forEach(function($cap){
    if( state.mushrooms ) $cap.style.visibility = "visible"
    else $cap.style.visibility = "hidden"
  })
}

function renderGreenPeppers() {
  // Iteration 1: set the visibility of `<section class="green-pepper">`
  document.querySelectorAll('.green-pepper').forEach( function($gren_pep){
    if( state.greenPeppers) $gren_pep.style.visibility = "visible"
    else $gren_pep.style.visibility = "hidden"
  })
}
