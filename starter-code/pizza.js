...
let ing = Object.keys(ingredients);

function renderButtons() {
  // Iteration 3: add/remove the class "active" of each `<button class="btn">`
  const ingBtn = document.querySelectorAll(".btn");

  for (let i = 0; i < ing.length; i++) {
    if (state[ing[i]]) {
      ingBtn[i].classList.add("active");
    } else {
      ingBtn[i].classList.remove("active");
    }
  }
}
...
function renderPrice() {
  ...
  // for i in ing, if state{i}
  for (let elem of ing) {
    if (state[elem]) {
      sum += ingredients[elem].price;
    }
  }
