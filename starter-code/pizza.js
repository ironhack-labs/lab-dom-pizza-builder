...
function renderButtons() {
  ...
  const $pepperonniBtn = document.querySelector(".btn.btn-pepperonni");
  if (state.pepperonni) {
    $pepperonniBtn.classList.add("active");
  } else {
    $pepperonniBtn.classList.remove("active");
  }
...
function renderPrice() {
  ...
  const $container = document.querySelector(".price ul");
  $container.innerHTML = "";
  ...
  for (element in state) {
    if (state[element]) {
      ...
      $container.innerHTML += `<li>$${ingredients[element].price} ${ingredients[
        element
      ].name.toLowerCase()}</li>`;
    }
  }
