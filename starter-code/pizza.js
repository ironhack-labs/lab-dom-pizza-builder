...
function renderButtons() {
  ...
  document
    .querySelectorAll(".btn.btn-green-peppers")
...
function renderPrice() {
  ...
  for (let i = 0; i < ingredientsArr.length; i++) {
    let ingName = ingredientsArr[i];

    if (state[ingName]) {
      list += `<li>$${ingredients[ingName].price} ${ingredients[ingName].name}</li>`;
      totalPrice += ingredients[ingName].price;
    }
  }
